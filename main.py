    ...
    with open(file_path, 'r', encoding= 'utf-8') as f:
        for line in f:
            ...
            for item in range(count):
                ...
                ingrs['ingridient_name'], ingrs['quantity'], ingrs['measure'] = ingr.split('|')
    ...
    for dish_name in dishes:
        if dish_name in cook_book:
            for ings in cook_book[dish_name]:
                meas_quan_list = dict()
                if ings['ingridient_name'] not in ingr_list:
                    meas_quan_list['measure'] = ings['measure']
                    meas_quan_list['quantity'] = ings['quantity'] * person_count
                    ingr_list[ings['ingridient_name']] = meas_quan_list
                else:
                    ingr_list[ings['ingridiet_name']]['quantity'] = ingr_list[ings['ingridient_name']]['quantity'] + ings['quantity'] * person_count
        else:
